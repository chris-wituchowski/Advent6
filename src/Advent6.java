...
public class Advent6 {
    public static void main(String[] args) {
        ...
        String inputString = "turn on 0,0 through 999,999\n"
        ...
        int changedCellValue = 0;
        int num1s = 0;

        Pattern pullString = Pattern.compile("(on|off|toggle)");

        Pattern pullInt = Pattern.compile("(\\d+),+(\\d+) \\w+ (\\d+),+(\\d+)");

        Matcher matcherStr = pullString.matcher(input);
        Matcher matcherDig = pullInt.matcher(input);

        while (scanner.hasNextLine()) {
            input = scanner.nextLine();

            matcherStr = pullString.matcher(input);
            matcherDig = pullInt.matcher(input);

            while (matcherStr.find()) {
                matcherDig.find();

                row1 = Integer.parseInt(matcherDig.group(1));
                col1 = Integer.parseInt(matcherDig.group(2));
                row2 = Integer.parseInt(matcherDig.group(3));
                col2 = Integer.parseInt(matcherDig.group(4));

                toggleOnOff = matcherStr.group();

                for (int i = row1; i < row2; i++) {
                    for (int j = col1; j < col2; j++) {
                        currentCellValue = lightsGrid[i][j];
                        switch (toggleOnOff) {
                            case "on":
                                changedCellValue = 1;
                                break;
                        ...
                        lightsGrid[i][j] = changedCellValue;
                ...
                System.out.println(toggleOnOff);
        ...
        System.out.print(num1s);
            ...
            /*if (turnOn.matcherStr(input).find(Integer.parseInt(input))) {

                on++;
            }

            if (turnOff.matcherStr(input).find()) {
                off++;
            }

            if (toggle.matcherStr(input).find()) {
                toggleInt++;
            }*/
